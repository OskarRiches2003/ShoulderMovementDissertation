import { n as ansis_default } from "./constants-bP1agsp3.js";
import { n as logger } from "./diagnostics-B4HZHfKw.js";
...
async function start(options) {
	const { host } = options;
	const { getPort } = await import("./dist-CzWh64Rs.js").then((n) => n.t);
	const port = await getPort({
		host,
		port: options.port == null ? void 0 : +options.port,
		portRange: [9999, 15e3]
	});
	const { startStandaloneDevTools } = await import("./standalone-uW4k2zIt.js");
	const { createDevToolsMiddleware } = await import("./server-vEML1MtP.js").then((n) => n.n);
...
async function build(options) {
	...
	const { buildStaticDevTools } = await import("./build-static-ZFt0TSdg.js");
